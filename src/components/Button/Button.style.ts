...
export const Button = styled.button<{ size?: string; bold?: boolean }>`
  ...
  font-family: ${({ size }) => (size === 'normal' ? '`Konsolev Regular`, sans-serif' : '`Konsolev SemiBold`, sans-serif')};
  ...
  ${() => mediaQuery.tablet} {
    font-size: ${({ size }) => (size === 'small' ? 'var(--fsSmall4)' : size === 'normal' ? 'inherit' : 'var(--fsBase)')};
  }
...
export const ButtonLink = styled.a<{ size?: string; bold?: boolean }>`
  padding: ${({ size }) => (size === 'normal' ? '7px 25px' : '5px 20px')};
  transition: background-color 250ms cubic-bezier(0.4, 0, 0.2, 1) 0ms, box-shadow 250ms cubic-bezier(0.4, 0, 0.2, 1) 0ms,
    border 250ms cubic-bezier(0.4, 0, 0.2, 1) 0ms;
  font-family: ${({ size }) => (size === 'normal' ? '`Konsolev Regular`, sans-serif' : '`Konsolev SemiBold`, sans-serif')};
